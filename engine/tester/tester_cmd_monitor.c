...
test_cmd_monitor_end(cmd_monitor_descr *monitor)
{
    ...
    if (WIFEXITED(rc))
    {
        RING("Command monitor '%s' exited with status %d",
             monitor->command, WEXITSTATUS(rc));
    }
    else if (WIFSIGNALED(rc))
    {
        RING("Command monitor '%s' was terminated by signal %d",
             monitor->command, WTERMSIG(rc));
    }
    else
    {
        ERROR("waitpid() returned unexpected status");
        return TE_OS_RC(TE_TESTER, TE_ESHCMD);
    }
...
start_cmd_monitors(cmd_monitor_descrs *monitors)
{
    ...
    TAILQ_FOREACH(monitor, monitors, links)
    {
        ...
        {
            ERROR("Failed to enable TA command monitor(type:%s) for '%s': %r",
