...
#define TESTER_VALGRIND               (1LLU << 16)
/** Run scripts under GDB in interactive mode */
#define TESTER_GDB                    (1LLU << 17)
/** Flag to be used on run path only */
#define TESTER_RUNPATH                (1LLU << 18)
/** End of run path */
#define TESTER_RUN                    (1LLU << 19)
/** Run tests regardless requested path */
#define TESTER_FORCE_RUN              (1LLU << 20)

/*
 * These seems to be not used currently.
 */
#define TESTER_MIX_VALUES             (1LLU << 21)
#define TESTER_MIX_ARGS               (1LLU << 22)
#define TESTER_MIX_TESTS              (1LLU << 23)
#define TESTER_MIX_ITERS              (1LLU << 24)
#define TESTER_MIX_SESSIONS           (1LLU << 25)

/** Force testing flow logging to ignore run item name */
#define TESTER_LOG_IGNORE_RUN_NAME    (1LLU << 26)
/** Log list of known requirements to the log file */
#define TESTER_LOG_REQS_LIST          (1LLU << 27)
/** Is in prologue/epilogue */
#define TESTER_INLOGUE                (1LLU << 28)
/** Interactive mode */
#define TESTER_INTERACTIVE            (1LLU << 29)
/** Shutdown test scenario */
#define TESTER_SHUTDOWN               (1LLU << 30)
/** Break test session on Ctrl-C */
#define TESTER_BREAK_SESSION          (1LLU << 31)
