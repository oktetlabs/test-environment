 ...
 * 
 * $Id: xen_saved_migrate.c $
 */


#define TE_TEST_NAME "xen/xen_saved_migrate"
          ...
          uint8_t const *mac, struct sockaddr const *ip)
{
    ...
    if (tapi_cfg_xen_dom_u_set_status(pco_dst->ta, dom_u,
                                      "running") != 0)
    {
        ERR_FLG("Failed to start '%s' domU on %s", dom_u, pco_dst->ta);
        goto cleanup1;
    }
