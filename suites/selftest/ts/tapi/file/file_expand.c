...
main(int argc, char **argv)
{
    ...
    /*
     * FIXME: the template should actually be a parameter, but
     * due to OL bug #12847 a parameter inside package.xml
     * cannot contain shell-like variable references
     */
    te_string_append(&template, "%s=${%s}", varname, varname);
    ...
    CHECK_RC(tapi_file_expand_kvpairs(pco_iut->ta, template.ptr, NULL, &kvpairs,
                                      "%s", filename.ptr));
    ...
    CHECK_RC(tapi_file_expand_kvpairs(NULL, template.ptr, NULL, &kvpairs,
