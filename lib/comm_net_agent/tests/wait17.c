...
#define ATTACHMENT_SIZE         ((TOTAL_BUFFER_LENGTH / 8) * 3)
...
#define DECLARED_BUFFER_LENGTH  ((TOTAL_BUFFER_LENGTH) / 4)
...
local_station_proc(void *arg)
{
    ...
    /* now call the _wait() function four times */
    ...
    if (rc != 0)
    {
	fprintf(stderr, "ERROR: the fourth call of rcf_comm_agent_wait() "
...
/** @page test_rcf_net_agent_wait17 test_rcf_net_agent_wait() quadruple call border value check, big attachment
 *
