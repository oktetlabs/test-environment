...
#define COMMAND_SIZE            ((TOTAL_BUFFER_LENGTH) / 2)

/* The default size of the attachment generated by this test */
#define ATTACHMENT_SIZE         ((TOTAL_BUFFER_LENGTH) / 3)
...
#define TEST_BUFFER_SANITY()                                            \
    do {                                                          \
        if (DECLARED_BUFFER_LENGTH * 3 <= MESSAGE_SIZE ||                \
           MESSAGE_SIZE > TOTAL_BUFFER_LENGTH ||                       \
           COMMAND_SIZE <= DECLARED_BUFFER_LENGTH ||                       \
           MESSAGE_SIZE <= DECLARED_BUFFER_LENGTH * 2)                       \
        {                                                          \
               fprintf(stderr,                                            \
...
/** @page test_rcf_net_agent_wait10 test_rcf_net_agent_wait() triple call, big command
 *
 * @descr The remote station issues a message consisting of a large command 
 * (that would not fit in a single input buffer, but would do in two) and
 * an attachment, so that totally three input buffers are required to
 * room the total message. The first call of the function 
 * @b rcf_comm_agent_wait() must return TE_ESMALLBUF, the second call
 * must return TE_EPENDING, and the third call must succeed. The three
 * calls must make up the original message sent by the remote station.
 *
