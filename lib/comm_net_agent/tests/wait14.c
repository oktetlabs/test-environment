...
#define ATTACHMENT_SIZE       ((TOTAL_BUFFER_LENGTH / 3) * 3 - (COMMAND_SIZE))
...
#define TEST_BUFFER_SANITY()						  \
    do {								  \
	    ...
	    ATTACHMENT_SIZE >= DECLARED_BUFFER_LENGTH * 2)		  \
        {								  \
    	    fprintf(stderr,						  \
...
/** @page test_rcf_net_agent_wait14 test_rcf_net_agent_wait() triple call border value check, big attachment
 *
 ...
 * ETESMALLBUF, the second call must return ETEPENDING, and the third call
