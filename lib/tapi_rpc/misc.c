              ...
              int resource, const struct rlimit *rlim)
{
    ...
    in.resource = rpc_resource;
    if (rlim == NULL)
        return TE_RC(TE_RCF, EINVAL);

    in.rlim.rlim_val->rlim_cur = rlim->rlim_cur;
    in.rlim.rlim_val->rlim_max = rlim->rlim_max;

    rcf_rpc_call(rpcs, "setrlimit", &in, &out);

    CHECK_RETVAL_VAR_IS_ZERO_OR_MINUS_ONE(setrlimit, out.retval);

    TAPI_RPC_LOG("RPC (%s,%s)%s: setrlimit(%s, {%u, %u}) -> %d (%s)",
                 rpcs->ta, rpcs->name, rpcop2str(op),
              ...
              int resource, struct rlimit *rlim)
{
    ...
    if ((out.rlim.rlim_val = malloc(sizeof(struct tarpc_rlimit))) == NULL)
    {
        ERROR("Cannot allocate memory for rpc_getrlimit() call");
        return TE_RC(TE_RCF, ENOMEM);
    }
    out.rlim.rlim_len = sizeof(struct tarpc_rlimit);
