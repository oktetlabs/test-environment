                          ...
                          const char* data_id_str, double *value)
{
    ...
    rc = cfg_get_instance_string_fmt(&value_str,
                                     "/agent:%s/hardware:/sensor:%s/data:%s"
                                     "/value:",
                                     ta, sensor_name, data_id_str);
    if (rc != 0)
    {
        ERROR("Failed to get value property of sensor %s, data_id %s:"
              " error %r", sensor_name, data_id_str, rc);
        return rc;
    }
                        ...
                        const char* data_id_str, double *value)
{
    ...
    if (rc != 0)
    {
        ERROR("Failed to get min property of sensor %s, data_id %s:"
              " error %r", sensor_name, data_id_str, rc);
        return rc;
    }
                        ...
                        const char* data_id_str, double *value)
{
    ...
    if (rc != 0)
    {
        ERROR("Failed to get max property of sensor %s, data_id %s:"
              " error %r", sensor_name, data_id_str, rc);
        return rc;
    }
                              ...
                              const char* threshold_id, double *value)
{
    ...
    if (rc != 0)
    {
        ERROR("Failed to get threshold property of sensor %s, data_id %s,"
              " threshold_id %s: error %r", sensor_name, data_id_str,
              threshold_id, rc);
        return rc;
    }
