...
prepare_hosts(tapi_env_hosts *hosts, cfg_nets_t *cfg_nets)
{
         ...
         host = host->links.cqe_next)
    {
        ...
        /* Get time to wait for ARP changes propogation */
        rc = cfg_find_fmt(&handle, "/local:%s/arp_sync:", host->ta);
        if (rc != 0)
        {
            if (TE_RC_GET_ERROR(rc) == TE_ENOENT)
            {
        ...
        {
            type = CVT_INTEGER;
            rc = cfg_get_instance(handle, &type, &host->arp_sync);
            if (rc != 0)
            {
                ERROR("Failed to get instance by handle 0x%x: %r",
                      handle, rc);
        ...
        /* Get time to wait for routing table changes propogation */
        rc = cfg_find_fmt(&handle, "/local:%s/route_sync:", host->ta);
        if (rc != 0)
        {
            if (TE_RC_GET_ERROR(rc) == TE_ENOENT)
            {
                type = CVT_INTEGER;
                rc = cfg_get_instance_fmt(&type, &host->route_sync,
                                          "/local:/route_sync:");
                if (TE_RC_GET_ERROR(rc) == TE_ENOENT)
                {
                    host->route_sync = 0;
        ...
        {
            type = CVT_INTEGER;
            rc = cfg_get_instance(handle, &type, &host->route_sync);
            if (rc != 0)
            {
                ERROR("Failed to get instance by handle 0x%x: %r",
                      handle, rc);
