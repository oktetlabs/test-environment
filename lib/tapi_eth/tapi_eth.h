...
#ifndef TAPI_DHCP_H__
#define TAPI_DHCP_H__
...
extern int tapi_eth_csap_create_with_mode(const char *ta_name, int sid,
                               const char *device, uint8_t recv_mode,
                               const uint8_t *remote_addr, 
                               const uint8_t *local_addr, 
                               const uint16_t *eth_type_len, 
                               csap_handle_t *eth_csap);
 ...
 * received. 
 *
 * If all packets are successfully sent before trrecv_stop command, CSAP status
 * become 'CSAP_IDLE', i.e. free for new traffic activity, and no trsend_stop
 * command is need. 
 *
 * If some packet was not sent by cause of some error, CSAP status become 
 * CSAP_ERROR and CSAP waits 'trsend_stop' command to clear resources, 
 * rc code to trsend_stop is error code related to packet sending failure. 
 * In this case 'trsend_stop' command should be sent by 'rcf_ta_trsend_stop' 
 * function. 
 *
 * @param ta_name  - Test Agent name
 * @param sid      - RCF session identifier
