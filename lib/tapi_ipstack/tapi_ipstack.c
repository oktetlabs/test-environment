...
ndn_udp4_dgram_to_plain(asn_value_p pkt, udp4_datagram **udp_dgram)
{
    ...
    if ((*udp_dgram = (struct udp4_datagram *)malloc(sizeof(**udp_dgram))) == NULL)
        return ENOMEM;
...
tapi_udp4_prepare_tmpl_file(const char *fname, const udp4_datagram *dgram)
{
    ...
    fprintf (f, "               dst-addr plain:{%02x, %02x, %02x, %02x} } }",
                ip_addr_p[0], ip_addr_p[1], ip_addr_p[2], ip_addr_p[3]); 
                         ...
                         csap_handle_t *udp_csap)
{
    ...
    if (rc == 0)
    {
        ...
        rc = asn_write_value_field(csap_ip4_level, &loc_addr, sizeof(loc_addr), 
                                "local-addr.#plain"); 
        rc = asn_write_value_field(csap_ip4_level, &rem_addr, sizeof(rem_addr), 
                                "remote-addr.#plain");
        rc = asn_write_component_value(csap_level_spec, csap_ip4_level, "#ip4"); 
    }
