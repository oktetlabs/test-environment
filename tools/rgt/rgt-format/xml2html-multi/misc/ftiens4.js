...
{ 
  ...
  if (this.getID()!=a1.getID() && PRESERVESTATE && !this.isOpen) //closing
  {
...
{ 
  ...
  if (folderObj.id!=a1.id && PRESERVESTATE)
  {
    currentOpen = GetCookie("clickedFolder")
    if (currentOpen == null)
      currentOpen = ""
...
function clickOnLink(clickedId, target, windowName) {
    ...
    if (isLinked(target)) {
        /* Updated by oleg to corect for conquesror */
        if (supportsDeferral)
            window.open(target,windowName);
        else
        {
            var cmd = "parent." + windowName + ".location.href='" + target + "'";
            eval(cmd);
        }
    }
}
...
function CookieBranding(name) {
  if (typeof a1.treeID != "undefined")
    return name+a1.treeID //needed for multi-tree sites. make sure treeId does not contain cookieCutter
  else
    return name
}
...
{ 
  ...
  /*
  alert("family:" + navigator.family + ", version: " + navigator.version + 
        ", OS: " + navigator.OS + "browser: " + navigator.org + ", browserVersion: " + browserVersion);
   */
  supportsDeferral = ((navigator.family=='ie4' && navigator.version >= 5 && navigator.OS != "mac") || (browserVersion == 3 && navigator.version != 0));
  ...
  a1.initialize(0, true, "") 
  if (supportsDeferral && !STARTALLOPEN) {
      a1.renderOb(null) //delay construction of nodes
  }

  else {
    renderAllTree(a1, null);

    if (PRESERVESTATE && STARTALLOPEN)
      storeAllNodesInClickCookie(a1)
