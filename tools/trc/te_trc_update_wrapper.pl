...
my $def_tester_conf = "tester.conf.sockapi-ts";
my $def_trc_db = "trc/trc-sockapi-ts.xml";
...
sub help_info
{
    print "trc_update.pl - Script to interact with TRC update tool.\n";
    print "      --trc-mv                Move result in TRC DB\n";
    my $rc = system("te_trc_update.pl --help");
    exit $rc;
}
...
my $rc = system("te_trc_update.pl ".$opts);

if ($rc == 0 && $trc_mv == 1)
{
    $rc = system($script_dir."/trc_mv $trc_result");
}
