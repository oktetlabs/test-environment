...
extern int charno;		/* GCT */

#define gct_getc(filep)   	(charno++, getc(filep))		  /* GCT */
#define gct_ungetc(c, filep)	(charno--, ungetc((c), (filep)))  /* GCT */
...
/* keep track of lineno most recently counted as a non-comment line */
static int Gct_curlineno;

/* keep track of the number of non-comment source lines seen in main_input */
int Gct_non_com_lines; 
...
{
  extern int Gct_ignore_tokens;	/* GCT: Variable aus c-parse.y */
  register int c;
  ...
  Gct_ignore_tokens++;
  gct_ungetc (skip_white_space (c), finput); /* GCT */
  Gct_ignore_tokens--;
}
  ...
  if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z'))
    {
      if (c == 'p')
	{
	  if (gct_getc (finput) == 'r' /* GCT */
	      && gct_getc (finput) == 'a' /* GCT */
	      && gct_getc (finput) == 'g' /* GCT */
	      && gct_getc (finput) == 'm' /* GCT */
	      && gct_getc (finput) == 'a' /* GCT */
	      && ((c = gct_getc (finput)) == ' ' || c == '\t' || c == '\n')) 
	    {
	      /* GCT */
	      gct_make_end_note(gct_build_pragma(finput), Gct_all_nodes);
	      c = '\n'; 
              /* End GCT */
	...
	{
	    if (gct_getc (finput) == 'e'      /* GCT */
		&& gct_getc (finput) == 'f'   /* GCT */
		&& gct_getc (finput) == 'i'   /* GCT */
		&& gct_getc (finput) == 'n'   /* GCT */
		&& gct_getc (finput) == 'e'   /* GCT */
		&& ((c = gct_getc (finput)) == ' ' || c == '\t' || c == '\n'))
	    {
...
linenum:
  ...
  if (token == CONSTANT
      ...
      if (main_input_filename == 0) 
	{
	  ...
	  main_input_filename = input_filename; /* Old code */
 	  gct_set_file_context(main_input_filename, Gct_test_dir);
	  gct_init();
	} 

      /* GCT */
      gct_make_end_note(gct_build_line_note(input_filename, lineno+1), Gct_all_nodes);
...
readescape (ignore_ptr)
     int *ignore_ptr;
{
  register int c = gct_getc (finput);
  register int code;
...
/* GCT
 * This hashes a node.  I include the node type in the hash to avoid
 * pathological equalities like the old "a =- 5  vs.  a = -5".  Note that
 * the node type in this case is always GCT_OTHER.  That doesn't matter
 * because the hashing is order-dependent.  That is, a hash of
 *
 * 	GCT-OTHER a GCT-OTHER = - GCT-OTHER 5
 *
...
{
  int i;
  extern int Gct_function_hashval;	/* GCT: Variable aus c-parse.y */

  GCT_HASH(Gct_function_hashval, (int) node->type);
  if (node->text)
    {
      for (i = 0 ; i < node->textlen ; i++)
	GCT_HASH(Gct_function_hashval, (int) (node->text[i]));
    }
  else
    {
      warning("Node passed to gct_hash_node has no text.\n");
    }
}

/*
 * GCT Note: EVERY token that's read has a TEXT field attached.  This is
 * redundant -- for many tokens, TEXT is redundant with the TYPE field
 * (consider WHILE).  However, this saves having to write special case
 * code here in yylex and also in the printing code.  
 */
...
yylex ()
{
  ...
  int wide_flag = 0;
  int objc_flag = 0;
  int starting_char;		/* GCT:  First character of token */
  int gct_nulls_in_string;	/* GCT:  How many nulls in current string. */
  				/* NOTE:  Nulls in character constants are */
  				/* handled differently. */
  extern int charno;		/* GCT: Variable aus c-parse.y */
  extern int Gct_ignore_tokens;	/* GCT: Variable aus c-parse.y */
  Gct_ignore_tokens++;		/* GCT:  Ignore on recursive calls. */
  yylval.code = LAST_AND_UNUSED_TREE_CODE;	
				/* GCT:  Don't retain previous value
				   into tests at end of routine. */
 ...
 found_nonwhite:
  starting_char = charno;	/* GCT */
  gct_nulls_in_string = 0;	/* GCT */
...
/*  yylloc.first_line = lineno; */
  ...
  switch (c)
    ...
    case '$':
      if (dollars_in_ident)
	goto letter;
      return '$';	/* GCT:  Causes error -- no need to save node. */
	...
	if (c == '\'')
	  {
	    error("GCT does not accept wide character constants (L'c').\n");
	    wide_flag = 1;
	    goto char_constant;
	  }
	if (c == '"')
	  {
	    error("GCT does not accept wide strings (L\"string\").\n");
            wide_flag = 1;
...
/* The second argument, machine_mode, of REAL_VALUE_ATOF tells the
   desired precision of the binary result of decimal-to-binary conversion.  */
	...
	while (1)
	  ...
	  tryagain:
		...
		pedwarn("GCT:  Newline in character constant means line number off by one.");
		lineno++;
	      }
#ifdef MAP_CHARACTER
	    else
	...
	/*
	 * GCT:  put full character constant in token_buffer.  This is a
	 * problem, in that the value returned here and stored forever
	 * more is a character constant -- something that's clumsy to
	 * manipulate.  A possible solution is to let the first character
	 * (') be a CHARACTER tag and have the second character be the
	 * actual value.  The printer would print it correctly.
	 *
	 * Note that we can't use the node's TYPE field for the tag,
	 * because the lexer must not set that field to anything other
	 * than GCT_OTHER.  The parser assigns all other values, and
	 * there is code that depends on this (scanning for GCT_OTHER to
	 * find an unparsed statement).  It might be better to just burn
	 * a bit to denote "parsed" and let the lexer set the type in
	 * some cases.
	 *
	 * Note that the current handling of characters avoids the
	 * "null in string" problems that strings have.  Change this code,
	 * and you'll have to deal with that problem.  (character tests
	 * include null characters.)
	 */
	if (maxtoken < 10) /* This will doubtless never happen. */
	  extend_token_buffer(token_buffer);

	if ('\\' == result)
	  {
	    sprintf(token_buffer, "'\\\\'", result);	    
	  }
	else if ('\'' == result)
	  {
	    sprintf(token_buffer, "'\\''", result);	    
	  ...
	  {
	    sprintf(token_buffer, "'%c'", result);
	  }
	else
	  {
	    sprintf(token_buffer, "'\\%o'", result);
	  }
	value = CONSTANT;
	break;
      }
	...
	while (c != '"' && c >= 0)
	  {
		...
		pedwarn("GCT:  Newline in string means line number off by one.");
		lineno++;
	      }
	...
	*p = 0;
	...
	token_buffer[1] = c1 = gct_getc (finput);
	token_buffer[2] = 0;
...
/*  yylloc.last_line = lineno; */
  ...
  if (0 == Gct_ignore_tokens)
    {
      if (! end_of_file)
	{
	  /* Have to handle shift-type operators specially, since token_buffer
	     doesn't contain all the characters. */

	  char * string_to_save = token_buffer;
	  gct_node new_node;

	  if (LSHIFT_EXPR == yylval.code && ASSIGN == value)
	    string_to_save = "<<=";
	  else if (LSHIFT_EXPR == yylval.code)
	    string_to_save = "<<";
	  else if (RSHIFT_EXPR == yylval.code && ASSIGN == value)
	    string_to_save = ">>=";
	  else if (RSHIFT_EXPR == yylval.code)
	    string_to_save = ">>";
	  new_node = gct_node_from_string(string_to_save,
					  gct_nulls_in_string,
					  input_filename,
					  lineno, starting_char);
	  gct_hash_node(new_node);
	  gct_add_last(&Gct_all_nodes, new_node);
	  
	  if (Gct_curlineno != lineno &&
	      strcmp(input_filename, main_input_filename) == 0) {
              Gct_curlineno = lineno;
              Gct_non_com_lines++;
          }
	}
    }
