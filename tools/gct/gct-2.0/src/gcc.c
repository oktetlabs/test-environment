...
void fatal ();            /* gct: changed from static void to void */
static void error ();
void warning ();		/* gct */
void fancy_abort ();
char *xmalloc ();
...
or with constant text in a single argument.
 ...
 %5	Like %g, but the temporary file has no directory component. (GCT)
 	GCT temporary files go in the compiler's execution directory.
 	%5 also has the effect of %d.  
 %6	The next compiler to use after GCT -- "cc" by default.  Set by -test-cc.
 %7	One object to be linked in to form the final executable (GCT)
 %8	Another object to be linked in to form the final executable (GCT)
 ...
 %9	process GCT_COMPILER_SPEC as a spec.	*** gct ***
...
/* GCT */
/* config.h can define GCT_COMPILER_SPEC to provide extra args to 
   the compiler called if OPT_PRODUCE_OBJECT set.  (The %6 compiler). */
#ifndef GCT_COMPILER_SPEC
#define GCT_COMPILER_SPEC ""
#endif
...
#endif
/* GCT */
/* This defines which of GCT's switches (all multi-letter) take arguments. */
#define GCT_WORD_SWITCH_TAKES_ARG(STR)  \
...
/*
 * GCT can be run in one of these FILE_STYLES:
 * 1.  Instrument, then compile a file.
 * 2.  Just compile the file, without instrumenting.  (Instrumentation is
 *     turned off for this file.)
 * 3.  Instrument only.  (Backwards compatibility; result is a C file to
 *     compile later.)
 * 4.  Don't do anything at all (Backwards compatibility.)
 */

#define GCT_STYLE_OLD_NO_INST		0x00	/* Don't do anything */
#define GCT_STYLE_OLD_INST		0x01	/* Instrument, no compile. */
#define GCT_STYLE_NEW_NO_INST		0x02	/* Compile only */
#define GCT_STYLE_NEW_INST		0x04	/* Instrument, compile */
#define GCT_STYLE_NEW  GCT_STYLE_NEW_INST | GCT_STYLE_NEW_NO_INST
#define GCT_STYLE_OLD  GCT_STYLE_OLD_INST | GCT_STYLE_OLD_NO_INST
...
gct_set_style()
{
  if (ON == gct_option_value(OPT_PRODUCE_OBJECT))
    {
      Gct_style = ((OFF == gct_option_value(OPT_IGNORE))
		   ? GCT_STYLE_NEW_INST
		   : GCT_STYLE_NEW_NO_INST);
    }
  else	/* Old style file handling. */
    {
      Gct_style = ((OFF == gct_option_value(OPT_IGNORE))
		   ? GCT_STYLE_OLD_INST
		   : GCT_STYLE_OLD_NO_INST);
    }
}

...
struct compiler
{
  ...
  int gct_style;		/* Under which GCT style this spec applies. */
...
/*
 * This spec applies to new-style instrumentation where the file is not
 * to be instrumented.  It just passes the file, and all its arguments
 * to the C compiler.   For the old style, the same effect is gotten
 * by simply having no matching spec.  
 *
 * It would seem that we don't need to use passcc.  However, 
 * 1.  We need to identify .o files as deletable and/or linker arguments,
 *     so the driver knows what to do later.
 * 2.  In the existing GCC framework, you do that by specially flagging
 *     the files as they're used as arguments.
 * 3.  So we have to call something with both -c and the object files names;
 *     but we can't assume that 'cc' can handle both -c and -o.
 * 4.  So we have to use a program that can.
 *
 * Could use callcc, but I want to be sure that if the original command
 * line was
 *
 * 	cc -c -o myout.o orig.c
 *
 * that no existing file orig.o is overwritten or deleted.  Further, we
 * must ensure that 
 *
 * 	cc -c -o orig.o orig.c
 *
 * as silly as it is, does the right thing.  (A naive use of callcc would
 * produce orig.o, then delete it.
 */
#define GCT_NEW_PASSTHROUGH \
"passcc %{c:%{o*}%{!o*:-o %w%b.o}}%{!c:-o %d%w%b.o} \
	 %6 %{c:%{o*}} \
	 %{C} %{D*} %{U*} %{I*} %{M*} %{i*} %{trigraphs} %{E}\
	 %{Y*} %{d*} %{m*} %{f*} %{a} %{S}\
 ...
 *
 * 1.  -test-traditional is used to make GCT handle "traditional" code.
 * It differs from -traditional in that it is not passed to the %N C compiler.
 */
   ...
   "%{!M:%{!MM:%{!E:cc1 %{!pipe:%g.i} %1 \
		   ...
		   %{S:%W{o*}%{!o*:-o %b.s}}%{!S:-o %{|!pipe:%g.s}} |\n\
		      ...
		      %{c:%W{o*}%{!o*:-o %w%b.o}}%{!c:-o %d%w%u.o}\
                      %{!pipe:%g.s} %A\n }}}}"},
#endif
  {"@c", GCT_STYLE_NEW_INST,
   "gct-cpp -lang-c %{nostdinc*} %{C} %{v} %{A*} %{I*} %{P} %I\
	%{C:%{!E:%eGNU C does not support -C without using -E}}\
	%{M} %{MM} %{MD:-MD %b.d} %{MMD:-MMD %b.d}\
	-undef -D__GNUC__=2 %{ansi:-trigraphs -$ -D__STRICT_ANSI__} %{!undef:%{!ansi:%p} %P} %{trigraphs} \
        %c %{O*:%{!O0:-D__OPTIMIZE__}} %{traditional} %{ftraditional:-traditional}\
        %{traditional-cpp:-traditional} %{g*} %{W*} %{w} %{pedantic*}\
	%{test-traditional:-traditional} \
	%{test-cc*} -test-macrofile %g.mac \
	%{H} %{d*} %C %{D*} %{U*} %{i*}\
        %i %{!M:%{!MM:%{!E:%{!pipe:%g.i}}}}%{E:%W{o*}}%{M:%W{o*}}%{MM:%W{o*}} |\n",
   "%{!M:%{!MM:%{!E:gct-cc1 %{!pipe:%g.i} %1 \
		   %{!Q:-quiet} -dumpbase %b.c %{d*} %{m*} %{a}\
	 	   %{test-traditional:-traditional} %{test-metrics} \
                   %{test-map*}%{test-dir*}%{test-control*} -test-macrofile %g.mac \
		   %{g*} %{W*} %{w} %{pedantic*} %{ansi} \
		   %{traditional} %{v:-version} %{pg:-p} %{p} %{f*}\
		   %{aux-info*}\
		   %{pg:%{fomit-frame-pointer:%e-pg and -fomit-frame-pointer are incompatible}}\
		   -o %{|!pipe:%5.c} \n\
              gct-callcc %6 %{S:%{c:%{o*}%{!o*:-o %w%b.s}}%{!c:-o %w%b.s} %5.s}\
                        %{!S:%{c:%{o*}%{!o*:-o %w%b.o}}%{!c:-o %d%w%b.o} %5.o}\
		   %{Y*} %{d*} %{m*} %{f*} %{a} %{S}\
		   %{g} %{O*} %{W*} %{w} %{pedantic} %{ansi} %{traditional}\
		   %{v} %{gg} %{pg} %{p} %9\
		   %{!S:-c} %5.c }}}"},
  {"@c", GCT_STYLE_OLD_INST,
   "gct-cpp -lang-c %{nostdinc*} %{C} %{v} %{A*} %{I*} %{P} %I\
	%{C:%{!E:%eGNU C does not support -C without using -E}}\
	%{M} %{MM} %{MD:-MD %b.d} %{MMD:-MMD %b.d}\
	-undef %{ansi:-trigraphs -$ -D__STRICT_ANSI__} %{!undef:%{!ansi:%p} %P} %{trigraphs} \
   ...
   "%{!S:%{!M:%{!MM:%{!E:gct-cc1 %{!pipe:%g.i} %1 \
		   ...
		   %{pg:%{fomit-frame-pointer:%e-pg and -fomit-frame-pointer are incompatible}}\
		   -o %{|!pipe:%g.c} \n }}}}"},
  {"@c", GCT_STYLE_NEW_NO_INST, GCT_NEW_PASSTHROUGH},

  {"-",  GCT_STYLE_NEW, GCT_NEW_PASSTHROUGH},

  {".m", GCT_STYLE_NEW, GCT_NEW_PASSTHROUGH},

  {".h", GCT_STYLE_NEW, GCT_NEW_PASSTHROUGH},

  {".cc",  0, "@c++"},
  {".cxx", 0, "@c++"},
  {".C",   0, "@c++"},
  {"@c++", GCT_STYLE_NEW, GCT_NEW_PASSTHROUGH},

  {".i", 0, "@cpp-output"},
  {"@cpp-output", GCT_STYLE_NEW_INST,
   "gct-cc1 %i %1 %{!Q:-quiet} %{d*} %{m*} %{a}\
	      %{test-traditional:-traditional} %{test-metrics} \
              %{test-map*}%{test-dir*}%{test-control*} -test-no-macros \
	      %{g*} %{O*} %{W*} %{w} %{pedantic*} %{ansi} %{traditional}\
	      %{v:-version} %{pg:-p} %{p} %{f*}\
	      %{aux-info*}\
	      %{pg:%{fomit-frame-pointer:%e-pg and -fomit-frame-pointer are incompatible}}\
...
#if 0
  /*
   * Because old style instrumentation replaces the main_input_filename,
   * it will typically end up replacing the .c file the .i file is
   * derived from.  Then the makefile compiles the .i file, so no
   * instrumentation is added.  Therefore, this rule is left out.  So
   * far, no one seems to use .i files (or at least no one's complained
   * to date).
   *
   * Note that -O* is passed down to cc1, unlike in above specs.  We need
   * to use up the argument, to avoid "unrecognized argument" errors at
   * this level.  cc1 will complain if the argument is anything other
   * than -O.
   */
  {"@cpp-output", GCT_STYLE_OLD_INST,
   "cc1 %i %1 %{!Q:-quiet} %{d*} %{m*} %{a}\
	      ...
	      -o %{|!pipe:%g.c} \n"},
#endif
  {"@cpp-output", GCT_STYLE_NEW_NO_INST, GCT_NEW_PASSTHROUGH},

  {".ii", GCT_STYLE_NEW, GCT_NEW_PASSTHROUGH},

  {".s",  GCT_STYLE_NEW, GCT_NEW_PASSTHROUGH},
  {".S",  GCT_STYLE_NEW, GCT_NEW_PASSTHROUGH},
  /* Mark end of table */
  {0, 0}
};
...
/* Have gcc do the search for libgcc.a, but generate -L options as usual.  */
static char *link_command_spec = "\
%{!c:%{!M:%{!MM:%{!E:%{!S:%6 %X %{o*} %9 %{A} %{d} %{e*} %{m} %{N} %{n} \
			%{r} %{s} %{T*} %{t} %{u*} %{x} %{z}\
			%{p} %{g} %{O*} %d%7 %d%8 %{static:} %{L*} %D %o \n }}}}}";
#else
#ifdef LINK_LIBGCC_SPECIAL
/* Have gcc do the search for libgcc.a, and don't generate -L options.  */
static char *link_command_spec = "\
...
/* GCT:  the first two of these are arguments; the last is constructed. */
char *Gct_test_dir = ".";
char *Gct_test_map = "gct-map";
char *Gct_test_control = "gct-ctrl";
char *Gct_full_control_file_name;


/* This argument is used ONLY when running tests of the control file.
   (Since putting (option show_ctrl) in the control file would interfere with
   some of the tests. */
int Gct_test_show_control = 0;

/* This argument turns off linking of executables. */

int Gct_test_nolink = 0;

/* This describes GCT switches used ONLY in the driver program.
   It prevents "unrecognized option" error messages. */

char *Gct_driver_switches="%{test-nolink} %{test-show-ctrl}";
  

/*
 * This is the argument to the -test-cc option.  It is used to select the
 * C compiler to run after GCT does.  It is relevant both to new style
 * instrumentation (where the C compiler is called by GCT) and old
 * style (where the C compiler is called by the user on the replaced
 * source). 
 */
char *Gct_test_cc = "cc";


/* Note that this is only called at link time - that is, once - so there's
   no reason to worry about storage. */
void
gct_call_remap()
{
  int length;
  char *system_buffer;
  char *kit1 = "gct-kit1-here";	/* If in libdir, expansion kit installed. */
  
#define GCT_REMAP_STRICT_OPTION " -strict "
#define GCT_REMAP_FORGET_OPTION " -forget "
#define GCT_REMAP_VERBOSE_OPTION " -v "
  int strict = (ON == gct_option_value(OPT_REMAP_STRICT));
  int forget = (ON == gct_option_value(OPT_REMAP_FORGET));
  int verbose = (ON == gct_option_value(OPT_REMAP_VERBOSE));

  length = 200 + strlen(Gct_test_dir) + strlen(Gct_test_map);
  if (strict)    length += strlen(GCT_REMAP_STRICT_OPTION);
  if (forget)    length += strlen(GCT_REMAP_FORGET_OPTION);
  if (verbose)   length += strlen(GCT_REMAP_VERBOSE_OPTION);

  if (kit1 == find_file(kit1))
    {
      return;
    }
  
  system_buffer = (char *) xmalloc(length);

  sprintf(system_buffer, "gct-remap %s %s %s -test-dir %s -test-map %s",
	  (strict ? GCT_REMAP_STRICT_OPTION : ""),
	  (forget ? GCT_REMAP_FORGET_OPTION : ""),
	  (verbose ? GCT_REMAP_VERBOSE_OPTION : ""),
	  Gct_test_dir, Gct_test_map);
  if (verbose_flag || ON == gct_option_value(OPT_VERBOSE))
    {
      fprintf(stderr, "%s\n", system_buffer);
    }
  if (0 != system(system_buffer))
    {
      error("Invocation of gct-remap failed.  Possible problems:");
      error("1.  gct-remap discovered an error.");
      error("2.  GCT was incorrectly installed.");
      fatal("3.  The GCT executables are not in your $PATH.");
    }
}  
	  

/*
 * Compile one of the auxiliary files (gct-ps-defs.c, gct-write.c).
 * The source is in the -test-dir directory; the result is in the current
 * directory, as is usual with compilation.
 */
void
gct_compile_aux(file)
     char *file;
{
  int length = 100 + strlen(Gct_test_cc) + strlen(Gct_test_dir) + strlen(file);
  char *system_buffer = (char  *) xmalloc(length);

  sprintf(system_buffer, "%s -c %s",
	  Gct_test_cc,
	  gct_expand_filename(file, Gct_test_dir));
  ...
  if (0 != system(system_buffer))
    {
      fatal("Compilation of GCT auxiliary file %s failed.", file);
    }
}
...
/* These two variables have the same purpose as the previous.  The difference
   is that they're in the same directory as the source.  This is because they're
   compiled *as* the source, and we want include files to be resolved exactly
   as they'd be for the original file.  Note that this must be initialized
   once per original input file. */

char *Gct_temp_filename;
int Gct_temp_filename_length;
...
/* The same as the above routine, but for %5. */
void
choose_gct_temp_base ()
{
  if (Gct_temp_filename)	/* Free last allocation.  Big savings. */
    {
      free(Gct_temp_filename);
    }
  
  Gct_temp_filename = (char *) xmalloc(1+sizeof("ccXXXXXX"));
  strcpy(Gct_temp_filename, "ccXXXXXX");
  mktemp (Gct_temp_filename);
  Gct_temp_filename_length = strlen (Gct_temp_filename);
}

     ...
     int mode;
{
  ...
  int len = pprefix->max_len + strlen (name) + strlen (file_suffix) + 2;
  int tmp;
	...
	if (machine_suffix)
	  {
	    ...
	    if (pl->prefix[(tmp=strlen(pl->prefix)-1)] != '/') {
		strcat(temp+tmp, "/");
	    }
     ...
     char **argv;
{
  ...
  gcc_exec_prefix = getenv ("GCT_EXEC_PREFIX"); /* GCT */
      ...
      else if (argv[i][0] == '-' && argv[i][1] != 0 && argv[i][1] != 'l')
	{
	  ...
	  switch (c)
	    ...
	    default:
	      ...
	      if (!strcmp(p, "test-show-ctrl"))	/* GCT */
		  { Gct_test_show_control = 1; }
	      else if (!strcmp(p, "test-nolink"))  /* GCT */
		{ Gct_test_nolink = 1; }
	      else if (SWITCH_TAKES_ARG (c) > (p[1] != 0))
		i += SWITCH_TAKES_ARG (c) - (p[1] != 0);
	      else if (WORD_SWITCH_TAKES_ARG (p) 
		       || GCT_WORD_SWITCH_TAKES_ARG(p))  /* GCT */
		  {
		      /* GCT:  We're interested in some of these ourself. */
		      if (!strcmp(p, "test-dir"))
			  { if (argv[i+1][0]=='-' )	/* Check for common error */
			      { fatal("Missing -test-dir argument"); }
			  else
			      { Gct_test_dir = argv[i+1]; }
			  }
		      else if (!strcmp(p, "test-control"))
			  { if (argv[i+1][0]=='-' )
			      { fatal("Missing -test-control argument"); }
			  else
			      { Gct_test_control = argv[i+1]; }
			  }
		      else if (!strcmp(p, "test-map"))
			  { if (argv[i+1][0]=='-' )
			      { fatal("Missing -test-map argument"); }
			  else
			      { Gct_test_map = argv[i+1]; }
			  }
		      else if (!strcmp(p, "test-cc"))
			  { if (argv[i+1][0]=='-' )
			      { fatal("Missing -test-cc argument"); }
			  else
			      { Gct_test_cc = argv[i+1]; }
			  }			/* End GCT Changes. */
     ...
     char *soft_matched_part;
{
  ...
  while (c = *p++)
	...
	switch (c = *p++)
	  ...
	  case '5':
	    obstack_grow (&obstack, Gct_temp_filename, Gct_temp_filename_length);
	    delete_this_arg = 1;
	    arg_going = 1;
	    break;

	  case '6':
	    obstack_grow (&obstack, Gct_test_cc, strlen(Gct_test_cc));
	    arg_going = 1;
	    break;

	  case '7':
	    if (ON == gct_option_value(OPT_REMAP))
	      gct_call_remap();
	    if (ON == gct_option_value(OPT_LINK_DEF))
		{
		  gct_compile_aux(GCT_PER_SESSION_DEFINITIONS);
		  obstack_grow (&obstack,
				GCT_COMPILED_PER_SESSION_DEFINITIONS,
				strlen(GCT_COMPILED_PER_SESSION_DEFINITIONS));
		  arg_going = 1;
		}
	    break;

	  case '8':
	    if (ON == gct_option_value(OPT_LINK_LOG))
		{
		  gct_compile_aux(GCT_WRITE_SOURCE);
		  obstack_grow (&obstack,
				GCT_COMPILED_WRITE_SOURCE,
				strlen(GCT_COMPILED_WRITE_SOURCE));
		  arg_going = 1;
		}
	    break;
	  ...
	  case '9':
	    value = do_spec_1 (GCT_COMPILER_SPEC, 0, NULL_PTR);
	    if (value != 0)
	      return value;
	    break;
     ...
     char **argv;
{
  ...
  if (verbose_flag)
    {
      fprintf (stderr, "gct version %s\n", version_string);
      if (n_infiles == 0)
	exit (0);
    }

  /* GCT:  Read the control file now. */
  Gct_full_control_file_name =
    gct_expand_filename(Gct_test_control, Gct_test_dir);
  gct_control_init(Gct_full_control_file_name);
  if (   ON == gct_option_value(OPT_SHOW_CTRL)
      || Gct_test_show_control)
    {
      fprintf(stderr, "Control file is \"%s\".\n", Gct_full_control_file_name);
      gct_print_control_file(stderr);
    }

  if (-1 == access(gct_expand_filename(Gct_test_map, Gct_test_dir), W_OK))
    {
      error("Map file is unwritable or does not exist.  Did you forget gct-init?");
      fatal("GCT cannot continue.");
    }
  ...
  for (i = 0; i < n_infiles; i++)
    {
      ...
      char buffer[GCT_BUFSIZE];		/* GCT:  use in system(3) */
      int system_result;		/* GCT:  the result of that call. */
      ...
      /* GCT: Create per-file tempfile. */
      choose_gct_temp_base();

      /* GCT: We'll be using the options that apply to this file. */
      gct_no_file_context();
      gct_set_file_context(input_filename, Gct_test_dir);

      /* GCT: We can now tell what style of file processing we want. */
      gct_set_style();

      if (ON == gct_option_value(OPT_VERBOSE))
	fprintf(stderr, "gct:  Processing %s with style %x.\n",
		input_filename, Gct_style);

      if (Gct_style & GCT_STYLE_OLD)
	{
	char *input_filename_suffix = input_filename;
	input_filename_suffix += input_filename_length -2;

	  if (   input_filename_length < 3
	      || input_filename_suffix[0] != '.'
	      || input_filename_suffix[1] != 'c')
	  {
	    /* Ignore anything but C source files. */
	    outfiles[i] = input_filename;
	    this_file_error = 0;
	    continue;	/* Fetch next file. */
	  }

	  /* Is it already instrumented? (only applicable if source changes) */
	  sprintf(buffer,
		  "/bin/grep __GCT_INSTRUMENTATION_TAG %s > /dev/null\n",
		  input_filename);
	  system_result = system(buffer);
	  if (system_result == 0)
	    {
	      fatal("%s is already instrumented.  You must run grestore first.", input_filename);
	    }
	  else if (((system_result>>8)&0377) != 1)  /* SysV WHIBYTE define not portable. */
	    {
	      fatal("Couldn't check %s to see if it's already instrumented; giving up.", input_filename);
	    }
	}
      ...
      if (cp)
	{
	  ...
	  if (ON != gct_option_value(OPT_GCT_ONLY))	/* GCT */
	    {
		value = do_spec (p);
		free (p);
		if (value < 0)
  ...
  /*
   * GCT:  We link only in the new style.  Note that we care about the
   * style at the global level, independently of what happened to
   * particular functions.
   */
  gct_no_file_context();
  if (   ON == gct_option_value(OPT_PRODUCE_OBJECT)
      && ON == gct_option_value(OPT_LINK)
      && ! Gct_test_nolink)
    {
  ...
  if (! linker_was_run && error_count == 0)
    ...
    }		/*** end if gct_option_value Gct_test_nolink ***/
...
lookup_compiler (name, length, language)
     ...
     char *language;
{
    struct compiler *cp, *gct_spec; /* GCT */
  int recognized = 0;	/*** GCT ***/
	  ...
	  if (cp->spec[0][0] == '@')
	    {
		 ...
		 Construct the new compiler spec.  */
	      ...
	      /*** GCT ***/
	      if (gct_spec = lookup_compiler (NULL_PTR, 0, language))
	        { 
                  bcopy (gct_spec->spec, new->spec, sizeof new->spec);	
		  return new;
		}	
	      else	
		return gct_spec;		/*** GCT  return NIL ***/
	    }
...
/* gct: changed from static void to void in 'fatal' & 'warning' */
...
/* GCT:  Added this so that all three programs use same routines. */
void
warning (va_alist)
     va_dcl
{
...
validate_all_switches ()
{
  ...
  /* GCT */
  p = Gct_driver_switches;			
  while (c = *p++)
    if (c == '%' && *p == '{')
      /* We have a switch spec.  */
      validate_switches (p + 1);

#ifdef GCT_COMPILER_SPEC
  p = GCT_COMPILER_SPEC;
  while (c = *p++)
    if (c == '%' && *p == '{')
      /* We have a switch spec.  */
