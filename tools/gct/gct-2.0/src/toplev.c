...
char *Gct_defs = "gct-defs.h";
char *Gct_full_defs_file;

char *Gct_per_session = GCT_PER_SESSION;
char *Gct_full_per_session_file;

char *Gct_per_session_definitions = GCT_PER_SESSION_DEFINITIONS;
char *Gct_full_per_session_definitions_file;

char *Gct_restore_log = GCT_RESTORE_LOG;
char *Gct_full_restore_log_file;
 
/* Where info about macro boundaries comes from. */
char *Gct_macro_file = DEFAULT_MACRO_FILENAME;
 
/* Set to 0 if there is no macro file (as would be the case for a .i file). */
int Gct_macro_file_exists = 1;
     ...
     char *name;
{
  ...
  /* GCT */
  /* In GCT, -o gives the name of the temporary file in which to place
     instrumented code.  We use the asm_out_file just opened under
     the alias Gct_textout.  Since we still have GCT's back end, we
     reopen asm_out_file on /dev/null to avoid wasting file system space. */
  {
    extern FILE *Gct_textout;
    
    Gct_textout = asm_out_file;
    asm_out_file = fopen ("/dev/null", "w");
    if (asm_out_file == 0)
      pfatal_with_name ("/dev/null");
  }
  ...
  if (main_input_filename == 0) {
    main_input_filename = name;
      ...
      /* GCT:  Now that we have the filename, push it on the stack. */
...
/*** gct ***/
/* When the following rule is included, system(3) calls fail with
   crt0: /usr/lib/ld.so mapping failure.  Sunos bug? */
#ifdef RLIMIT_STACK_GCT_REMOVED
...
#if 0
  /* GCT_DEBUGGING */
  fprintf(stderr, "NOTE:  Malloc debugging on.\n");
  malloc_debug(2);
#endif
...
You Lose!  You must define PREFERRED_DEBUGGING_TYPE!
	    ...
	    {
	      /* GCT */
	      /* In GCT, the -o argument is the name of the temporary file
	         in which to place instrumented code.  I use my own variable,
	         Gct_tempname, instead of asm_file_name - for clarity. */
	      extern char *Gct_tempname;

	      asm_file_name = argv[++i];
	      Gct_tempname = asm_file_name;
	    }
	  /* GCT */
          else if (!strcmp (str, "test-metrics")) 
            { 
              Gct_collect_metrics = 1;
            }
	    ...
	    {
	      Gct_test_control = argv[++i];
	    }
	  else if (!strcmp (str, "test-macrofile"))
	    {
	      Gct_macro_file = argv[++i];
	    }
	  else if (!strcmp (str, "test-no-macros"))
	    {
	      Gct_macro_file_exists = 0;
	    }
	  /* End GCT Changes */
	  else if (str[0] == 'G')
	    {
	    ...
	    error ("Invalid option `%s'", argv[i]);
	  /*
	   * GCT just issues warnings for invalid options -- this is a
	   * bad idea for options with arguments, but it lets the SysVR4
	   * kernel compile.
	   */
	}
      else if (argv[i][0] == '+')
...
/*** gct ***/
  /* GCT:  Initialize the various file names. */
  Gct_full_control_file_name = gct_expand_filename(Gct_test_control, Gct_test_dir);
  Gct_full_map_file_name = gct_expand_filename(Gct_test_map, Gct_test_dir);
  Gct_full_restore_log_file = gct_expand_filename(Gct_restore_log, Gct_test_dir);
  Gct_full_defs_file = gct_expand_filename(Gct_defs, Gct_test_dir);
  Gct_full_per_session_file = gct_expand_filename(Gct_per_session, Gct_test_dir);
  Gct_full_per_session_definitions_file = gct_expand_filename(Gct_per_session_definitions, Gct_test_dir);
  
  /* GCT:  Read the control file. */
