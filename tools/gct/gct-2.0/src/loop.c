...
static int try_modify_loop_termination ();
     ...
     rtx loop_end;
{
  ...
  /* Try to modify the termination conditional so that the conditional
     does not depend on the OVERFLOW status bits.  This allows more
     redundant tst insns to be eliminated.  */
  try_modify_loop_termination (loop_end);
...
/* Check to see if the loop termination condition can be turned into
   a condition which does not depend on the OVERFLOW status bits.

   Doing so for some loops will allow final to eliminate more useless
   tst insns.  Specifically this helps count down loops which have a
   non-negative start value and continue looping until they reach
   zero.  */
     ...
     rtx loop_end;
{
  ...
  /* Check the termination condition.  We might optimize if the
     termination condition checks (gt (cc0) (const_int 0)) and
     the loop is a count down loop.  */
  if (((GET_CODE (comparison) == GT
	&& GET_CODE (XEXP (comparison, 1)) == CONST_INT
	&& INTVAL (XEXP (comparison, 1)) == 0))
	...
	&& (INTVAL (bl->initial_value) %
	    (-INTVAL (bl->biv->add_val))) == 0)
      {
	rtx reg, jump_label, final_value, tem;

	/* Optimize by replacing the GT condition with NE.  This
	   eliminates the use of the OVERFLOW bit in the conditional
	   which allows final to delete more tst insns.  */
	...
	start_sequence ();
	emit_cmp_insn (reg, const0_rtx, NE, 0, GET_MODE (reg), 0, 0);
	emit_jump_insn (gen_bne (XEXP (jump_label, 0)));
	tem = gen_sequence ();
	end_sequence ();
