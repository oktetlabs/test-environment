...
/* GCT:  Many declarations moved to c-decl.h include file, also used in
   gct-decl.c */
#include <stdio.h>
#include "input.h"
#include "c-decl.h"
...
{ NORMAL,			/* Ordinary declaration */
  ...
  decl_contex_TYPENAME};        /* Typename (inside cast or sizeof)  */
...
void gct_write_metrics();

extern char *strrchr();


extern int Gct_collect_metrics;

      ...
      if (t != 0 && duplicate_decls (x, t))
	{
	 ...
	 standard built-in type.
      ...
      if (TREE_CODE (x) == TYPE_DECL)
		...
		{
		    /* GCT */
		    int code = TREE_CODE(TREE_TYPE(x));
		    if (UNION_TYPE == code || RECORD_TYPE == code) 
			fatal("GCT error: Unnamed struct or union got past parser.");
		    /* END GCT */
...
/* Given declspecs and a declarator,
   ...
   DECL_CONTEXT says which syntactic context this declaration is in:
     ...
     decl_contex_TYPENAME if for a typename (in a cast or sizeof).
      Don't make a DECL node; just return the ..._TYPE node.
     FIELD for a struct or union field; make a FIELD_DECL.
     BITFIELD for a field with specified width.
   INITIALIZED is 1 if the decl has an initializer.

   In the decl_contex_TYPENAME case, DECLARATOR is really an absolute declarator.
     ...
     int initialized;
{
  ...
  char *gct_flavor = 0;		/* GCT:  Name of flavor */
  extern tree Gct_last_decl;	/* GCT:  Last-named declarator. */
  int bitfield = 0;
  int size_varies = 0;
  ...
  for (spec = declspecs; spec; spec = TREE_CHAIN (spec))
      ...
      else if (TREE_CODE (id) == IDENTIFIER_NODE)
	{
	  ...
	  /* GCT -- store ID as possible flavor. */
	  gct_flavor = IDENTIFIER_POINTER(id);
  ...
  /*
   * GCT:  If the identifier is immediate (e.g., "int foo" or "mytype
   * foo"), we have the identifier's flavor.  Otherwise, (e.g., "int
   * *foo" or "int *(array[]())", we don't have any flavor at all, unless
   * the user later names it with a GCT comment.
   */
  ...
  if (decl_context == decl_contex_TYPENAME)
    {
      /* Note that the grammar rejects storage classes
	 in typenames, fields or parameters */
      if (pedantic && TREE_CODE (type) == FUNCTION_TYPE
...

  {
	...
	DECL_GCT_FLAVOR(decl) = gct_flavor;			/* GCT */
      }
    ...
    /* Stash declaration away so that GCT flavor comments can apply to it. */
    Gct_last_decl = decl; /* GCT */
    return decl;
  }
}

...
static int Gct_start_non_com_lines;
     ...
     int nested;
{
  ...
  named_labels = 0;
  shadowed_labels = 0;
  Gct_start_non_com_lines = Gct_non_com_lines;
  ...
  if (! nested)
    {
	 ...
	 pop_c_function_context and then reset via pop_function_context.  */
      ...
      /*
       * GCT:  This is a good time to write out the current function and
       * reclaim some space. 
       */
      assert_empty_mapfile_buffer();
      gct_write_list(Gct_all_nodes);
      gct_recursive_free_node(Gct_all_nodes);
      Gct_all_nodes = GCT_NULL_NODE;
    }
}

...
extern int gct_compute_mccabe();
extern int gct_compute_nonreducible();
static FILE *Gct_metrics = NULL;
static int Gct_metrics_error = 0;
void gct_write_metrics() 
{
    char metrics_name[1024];
    char *ptr;
    int mccabe;

    if (Gct_collect_metrics && !Gct_metrics_error && Gct_metrics == NULL) {
	strcpy(metrics_name, main_input_filename);
	if ((ptr = strrchr(metrics_name, '.')) != NULL &&
	    *(ptr+1) == 'c') {
	    ptr++;
	} else {
	    ptr = metrics_name + strlen(metrics_name);
	}
	strcpy(ptr,"met");
	*(ptr+3) = '\0';
	
	if ((Gct_metrics = fopen(metrics_name, "w")) == NULL) {
	    Gct_metrics_error++;
	    fprintf(stderr, "GCT: metrics file '%s' could not be opened\n",
		    metrics_name);
	    fprintf(stderr, "No metrics collected\n");
	}
    }

    if (Gct_collect_metrics && !Gct_metrics_error) {
	char *kind = "function";
	...
	if (current_function_decl == NULL)
	    fprintf (Gct_metrics, "At top level: %d\n", Gct_non_com_lines);
	else {
	    char *name = (*decl_printable_name) (current_function_decl, &kind);
	    mccabe = gct_compute_mccabe(Gct_all_nodes)+1;
	    fprintf (Gct_metrics, "%s `%s':\t%d\t%d\t%d\n", kind, name, 
		     Gct_non_com_lines-Gct_start_non_com_lines,
		     mccabe, gct_compute_nonreducible(Gct_all_nodes, 0)+1);
        }
	fflush(Gct_metrics);
    }
}
