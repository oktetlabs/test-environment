...
/*
 * This set of variables allows us to write a file telling where the
 * beginning and end of a macro lies.
 */

/*
 * This marks the beginning of a macroexpansion.  0 means we're in plain
 * text; >=1, in a macro.  Note that we don't care about nested expansions.
 */
int Gct_in_macro = 0;

char *Gct_macro_file = DEFAULT_MACRO_FILENAME;	/* Where macro information should go */
FILE *Gct_macro_stream;		/* The open file to write to. */
int Gct_macro_count = 0;	/* Number of entries written to the file. */

macro_data Gct_macro_data;	/* An in-core chunk of data about a macro. */


/* Prepare the macro data file for writing. */

gct_init_macro_data()
{
  Gct_macro_stream = fopen(Gct_macro_file, "w");
  if (NULL == Gct_macro_stream)
    {
      fprintf(stderr, "Failed to open macro file.\n");
      pfatal_with_name (Gct_macro_file);
    }
  /* Write placeholder; later overwritten with number of macros. */
  fprintf(Gct_macro_stream, "                   \n");
  Gct_macro_count = 0;
}


/*
 * Print a macro.  Currently, we print all the data in the Gct_macro_data
 * structure.  We also print, for debugging purposes, the expansion.
 * OP is the output file buffer pointer.
 *
 * Note that the data in Gct_macro_data is zero-based.  GCT's lexical
 * analyser tags tokens with 1-based offsets.  We do the conversion here.
 */

gct_print_macro_data(op)
     FILE_BUF *op;
{
  char *debug_buffer;	/* Holds expanded buffer. */
  char *rover;		/* Moves through that buffer. */
  
  int buf_str_len = Gct_macro_data.end - Gct_macro_data.start;

  fprintf(Gct_macro_stream, "%d %s %d %d\n",
	  strlen(Gct_macro_data.name),
	  Gct_macro_data.name,
	  Gct_macro_data.start + 1,
	  Gct_macro_data.end + 1);

  debug_buffer = (char *) xmalloc(buf_str_len + 1);
  bcopy(op->buf+Gct_macro_data.start, debug_buffer, buf_str_len);
  debug_buffer[buf_str_len] = '\0';

  /* Turns out that the internal representation can contain a carriage return
     if there's on in the arglist.  We strip these now. */

  
  for(rover = debug_buffer; '\0' != *rover; rover++)
    {
      if ('\n' == *rover)
	{
	  *rover = ' ';
...
{
  if (-1 == fseek(Gct_macro_stream, 0, 0))
    {
      fprintf(stderr, "Failed to seek to start of macro file.\n");
      pfatal_with_name(Gct_macro_file);
    }
  if (EOF == fprintf(Gct_macro_stream, "%d", Gct_macro_count))
    {
      fprintf(stderr, "Failed to print size of macro file.\n");
      pfatal_with_name(Gct_macro_file);
    }
  fclose(Gct_macro_stream);
}


/* Structures related to '#assert' and '#unassert' directives.
   Internally, '#assert predicate ( token_sequence )' is maintained as a
   list of lists of lists.  The topmost list(1) is a list of all active 
   predicates.  For each predicate there is a list(2) of token sequences.
   A token sequence is itself a list(3) of assert tokens.  For example:

   #assert X1 (A1)
   #assert X1 (A1 A2)
   #assert X2 (A1 B1 C1)

   Internally, these would be maintained as:
...
/* topmost list structure(1): */
struct assert_predicate {
   struct assert_predicate	*next;
   int				length;
   U_CHAR			*name;
   struct assert_sequence	*sequence_list;
};
typedef	struct assert_predicate A_PREDICATE;

/* for each predicate a list of token sequences(2): */
struct assert_sequence {
   struct assert_sequence	*next;
   int				token_count;
   struct assert_token		*token_sequence;
};
typedef	struct assert_sequence A_SEQUENCE;

/* each sequence is a list of assert tokens(3): */
struct	assert_token {
   struct assert_token		*next;
   int				length;
   U_CHAR			*name;
};
typedef	struct assert_token A_TOKEN;

/* head of assert predicate list */
A_PREDICATE	*assert_predicates = NULL; 

/* #assert related functions : */

int	     parse_predicate();
A_PREDICATE *find_predicate();
A_PREDICATE *insert_predicate();
void	     delete_predicate();
void	     free_predicate();

int	     parse_sequence();
A_SEQUENCE  *find_sequence();
A_SEQUENCE  *insert_sequence();
void	     delete_sequence();
void	     free_sequence();

void	     dump_predicates();
void	     dump_predicate();
void	     dump_sequence();
void	     dump_token();
     ...
     char **argv;
{
  ...
  for (i = 1; i < argc; i++) {
	...
	else if (!strcmp (argv[i], "-test-cc")) 	/* GCT */
	  /* Replace the default dir with the compiler-specific directory */
	  { char *new_default = (char *) xmalloc(strlen(GCC_INCLUDE_DIR) +
						 strlen(argv[i+1]) + 10);
	    strcpy(new_default, GCC_INCLUDE_DIR);
	    strcat(new_default, "/");
	    strcat(new_default, argv[++i]);
	    include_defaults[0].fname = new_default;
	    include_defaults[0].cplusplus = 0;		/*** GCT ***/
	  }
	else if (!strcmp (argv[i], "-test-macrofile")) 	/* GCT */
	  Gct_macro_file = argv[++i];
	break;
	...
	break;
      ...
      case 'v':
	fprintf (stderr, "GCT CPP version %s", version_string);
     ...
     int length;
{
  ...
  /* GCT - don't record macros within evaluation of an #if or #elseif expr. */
  Gct_in_macro++;
...
/* GCT:  We make sure OUTPUT_LINE_COMMANDS is defined, because that's
   what K&R2 use. */
...
#endif
#if 0
  /* GCT:  We suppress this token because older versions of GCC choke on it. */
     ...
     FILE_BUF *op;
{
  ...
  if (1 == Gct_in_macro)
    {
      Gct_macro_data.name = (char *) hp->name;
      Gct_macro_data.start = op->bufp - op->buf;
    }
  /* END GCT */
