...
* ``rebootable`` - Tell RCF whether Test Agent can be rebooted or not (possile values are yes or no, the default is no). If this attribute is not enabled, :cref:`rcf_ta_reboot` function from :ref:`API: RCF <doxid-group__rcfapi__base>` returns ``TE_EPERM`` error code;
...
Below element tells RCF that an agent should have a thread running :cref:`log_serial` function if TE_AGT_LOG_SERIAL is set. Arguments are passed to the thead function.
